...
def format_results(results: list[dict], show_importers: bool) -> None:
    if not results:
    ...
    for r in results:
        ...
        if show_importers and r["importers"]:
            print(f"{'':>{score_w + 2 + depth_w + 2 + imports_w + 2}}<- imported by:")
            for imp in r["importers"]:
                print(f"{'':>{score_w + 2 + depth_w + 2 + imports_w + 5}}{imp}")
            print()


def main():
    parser = argparse.ArgumentParser(
        description="Find widely-imported but deeply-nested Python modules."
    )
    parser.add_argument("root", help="Root directory of the Python codebase to analyze")
    parser.add_argument(
        "--min-imports", type=int, default=2,
        help="Minimum number of importers (default: 2)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-importers", action="store_true",
        help="Suppress the list of importing files"
    )
    ...
    format_results(filtered, show_importers=not args.no_importers)
